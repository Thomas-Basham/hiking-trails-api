...
def maps(request):
  ...
  for trail in hiking_trails:
    total_lat += float(trail.lat)
    total_lon += float(trail.lon)
    trail_count += 1
  map_center = [total_lat / trail_count, total_lon / trail_count]
  map = folium.Map(zoom_start=7, location=map_center)
  marker_cluster = MarkerCluster().add_to(map)
  for trail in hiking_trails:
    if current_user.id == trail.owner_id:
      edit_button = f'''
              <a style="float: right" href="{trail.id}/trail-update/" title="Edit Trail" target="_parent">
    ...
    html = f'''
          {edit_button}
          <h2 style="cursor: default" >{trail.trail_name.capitalize()}<h2/>
          <small  style="cursor: default">Added by <strong>{User.objects.get(id=trail.owner_id)}</strong> </small>
          <br>
          <a style="color:blue" href="{trail.google_maps_directions}" target="_blank" rel="nofollow" >Directions via Googlemaps <a/>
          <br>
          <a style="color:green" href="{trail.wta_link}" target="_blank">Link to WTA Page<a/>
    ...
    folium.Marker(location=location, tooltip=trail.trail_name.capitalize(), popup=popup,
